...
def test_ok_when_site_200_and_api_references_url() -> None:
    ...
    api = build_api_media_index(make_live([make_live_item(
        "https://www.thingelstad.com/2024/01/02/p.html",
        f'<img src="{asset_url}">',
    )]))
    probes = {asset_url: MediaProbe(url=asset_url, status=200, error=None,
                                    final_url=asset_url, size=1234)}
...
def test_site_missing_when_site_404_but_api_post_references_it() -> None:
    ...
    probes = {asset_url: MediaProbe(url=asset_url, status=404, error=None,
                                    final_url=asset_url)}
    findings = reconcile_media(bar=bar, probes=probes, api_index=api)
...
def test_size_mismatch_when_lengths_differ() -> None:
    ...
    api = build_api_media_index(make_live([make_live_item(
        "https://www.thingelstad.com/p.html", f'<img src="{asset_url}">',
    )]))
    probes = {asset_url: MediaProbe(url=asset_url, status=200, error=None,
                                    final_url=asset_url, size=4999)}
    findings = reconcile_media(bar=bar, probes=probes, api_index=api)
...
def test_external_broken_when_external_url_is_404() -> None:
    ...
    bar = make_bar(media_files=[], posts=[post])
    api = build_api_media_index(make_live([]))
    probes = {external: MediaProbe(url=external, status=404, error=None,
                                   final_url=external)}
    findings = reconcile_media(bar=bar, probes=probes, api_index=api)
...
def test_site_error_classifies_5xx_and_transport() -> None:
    ...
    api = build_api_media_index(make_live([]))
    probes = {asset_url: MediaProbe(url=asset_url, status=503, error=None,
                                    final_url=asset_url)}
    findings = reconcile_media(bar=bar, probes=probes, api_index=api)
    assert findings[0].classification == MediaClassification.SITE_ERROR

    probes = {asset_url: MediaProbe(url=asset_url, status=0, error="timeout",
                                    final_url=asset_url)}
    findings = reconcile_media(bar=bar, probes=probes, api_index=api)
    assert findings[0].classification == MediaClassification.SITE_ERROR
